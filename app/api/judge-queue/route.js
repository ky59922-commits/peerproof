...
export async function GET(request) {
  try {
    ...
    const { data: pendingAssessments, error: fetchError } = await supabaseAdmin
      .from("assessments")
      .select("id, candidate_degree, candidate_field, hr_notes, focus, candidate_nationality, work_experience, created_at, current_round, languages, proposed_slots(id, slot_time)")
      .eq("status", "pending")
