...
export default function MeetingRoom({ isJudge, nextPath, sessionId = "demo-session-1" }) {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      // 2. Peer connection (direct browser-to-browser)
      ...
      pc.onconnectionstatechange = () => {
        if (pc.connectionState === "connected") {
          ...
          startRecording(stream, remoteVideoRef.current?.srcObject);
          scheduleScreenshots();
        }
      ...
      // 3. Signaling over Supabase Realtime (no media passes through this — just text messages)
  ...
  function scheduleScreenshots() {
    const count = 5 + Math.floor(Math.random() * 4); // 5–8 screenshots
    const windowMs = 25 * 60 * 1000; // spread across ~25 min
    for (let i = 0; i < count; i++) {
      const delay = Math.random() * windowMs;
      screenshotTimersRef.current.push(setTimeout(captureScreenshot, delay));
    }
  }

  async function captureScreenshot() {
    // Always capture the candidate's face, regardless of who is running this code
    const videoEl = isJudge ? remoteVideoRef.current : localVideoRef.current;
    ...
    canvas.toBlob(async (blob) => {
      ...
      await supabase.storage.from("screenshots").upload(path, blob, { contentType: "image/jpeg" });
    }, "image/jpeg", 0.8);
  }
  ...
  async function endSession() {
    ...
    if (recorderRef.current && recorderRef.current.state !== "inactive") {
      ...
      if (blob.size > 0) {
        const path = `${sessionId}/audio-${Date.now()}.webm`;
        await supabase.storage.from("recordings").upload(path, blob, { contentType: "audio/webm" });
      }
    }
    screenshotTimersRef.current.forEach(clearTimeout);
    pcRef.current?.close();
